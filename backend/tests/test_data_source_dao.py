...
async def test_create_data_source(session, ds_data, user1):
    ...
    ds = await data_source_dao.create_data_source(session, ds_data)
...
async def test_get_data_source_by_id(session, ds_data, user1):
    ...
    result = await data_source_dao.get_data_source_by_id(session, ds.id)
...
async def test_get_data_source_by_id_not_found(session, user1):
    ...
    result = await data_source_dao.get_data_source_by_id(session, 99999)
...
async def test_get_data_source_by_owner(session, user1, user2):
    ...
    ds1 = await data_source_dao.create_data_source(
        session,
    ...
    ds2 = await data_source_dao.create_data_source(
        session,
        schemas.DataSourceCreate(
            name="Owner1 Source2",
            description="Owned by user1",
    ...
    ds3 = await data_source_dao.create_data_source(
        session,
        schemas.DataSourceCreate(
            name="Owner2 Source1",
    ...
    user1_sources = await data_source_dao.get_data_source_by_owner(session, user1)
    user2_sources = await data_source_dao.get_data_source_by_owner(session, user2.id)
...
async def test_update_data_source(session, user1):
    ds = await data_source_dao.create_data_source(
        session,
        schemas.DataSourceCreate(
            name="To Update",
    ...
    updated = await data_source_dao.update_data_source(session, ds.id, update_data)
...
async def test_update_nonexistent_data_source(session):
    ...
    with pytest.raises(ValueError):
        await data_source_dao.update_data_source(session, 99999, update_data)
...
async def test_delete_data_source(session, user1):
    ...
    deleted = await data_source_dao.delete_data_source(session, ds_id)
    assert deleted is not None
    #assert deleted.id == ds.id
    # Should not be found anymore
    result = await data_source_dao.get_data_source_by_id(session, ds_id)
    assert result is None

@pytest.mark.asyncio
async def test_delete_nonexistent_data_source(session):
    """Deleting a datasource with non-existing id returns None."""
    deleted = await data_source_dao.delete_data_source(session, 999999)
